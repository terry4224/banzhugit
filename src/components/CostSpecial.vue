...
<template>
  <div class="bbox">
    ...
    >
      ...
      <el-table-column label="计划预验收时间" width="120" prop="planPreinspectTime">
      </el-table-column>
...
const projectnames4 = ref(); // 定义父传递项目名称
const labelname4 = ref(); // 定义父传递lablename
...
watchEffect(() => {
  ...
  labelname4.value = props.labelname4;
  projectnames4.value = props.projectnames4;
});
...
const uuu = async () => {
  let res = await getData({
    method: "post",
    url: "/mgr/project-milepost/milepost4/page",
    data: {
      data: {
        projectName:projectnames4.value,
      },
...
//  监听是否被点击
watch(()=>labelname4.value,()=>{
 uuumounted();
})
// 监听项目名称是否改变
watch(()=>projectnames4.value,(newvalue,oldvalue)=>{
  uuu();
})
onMounted(()=>{
   uuumounted();
