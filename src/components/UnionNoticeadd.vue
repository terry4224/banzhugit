...
import {getbref,savebref} from '../api/unionnotice.js'
import { onBeforeUnmount, ref, shallowRef, onMounted } from "vue";
import { Editor, Toolbar } from "@wangeditor/editor-for-vue";
import {message} from '../utils/message.js';
import { ElMessage } from 'element-plus' // 引入消息提示
const emit = defineEmits(["changeNoticeAdd"]);
const noticeName = ref(""); // 通知标题的值
...
// const  ceshi = async ()=>{
// valueHtml.value = await getData({url:'/api/joke'});
...
const submit = async() => {
  ...
  return
    ...
    if(valueHtml.value.includes("br")){
     ElMessage({
    message: '请填写简介内容',
    type: 'warning',
  })
  ...
  const resut = await savebref({
	  url:'/mgr/cms-info/union/save',
	  method:'post',
	  data:{
  ...
  emit("changeNoticeAdd",false,true);
};
</script>
