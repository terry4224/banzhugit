<template>
	<div>
		<input type="file"  single @change="changeFiles" class="showipt" ref="ipt"  />
		<el-button style="margin-bottom: 4px;border: 0;" type="primary" @click="btnclick">
			<i class="fa fa-plus" aria-hidden="true"></i>批量上传
		</el-button>
	</div>
</template>
...
<script setup>
	...
	const emit = defineEmits(["getsuccess","getstate"]);
	const ipt = ref(); // 获取dom
	const btnclick = () => {
	...
	const changeFiles = async (e) => {
		...
		const res = await upload({
			method: "post",
			url: "/mgr/kpi-info/lastest/upload",
			data: formdata,
		});
		if(res.code = "00000"){
