...
function dateTime(arr) {
    for (let i = 0; i < arr.length; i++) {
        var timdedetail = arr[i].createDate;//假如是10位毫秒需要乘1000
        //获取年份
        ...
        arr[i].createDate=time
    }
}
// 上班打卡
function clockIn(arr){
    for (let i = 0; i < arr.length; i++) {
        var timdedetail = arr[i].beginCard;//假如是10位毫秒需要乘1000
        //获取年份
        var year = new Date(timdedetail).getFullYear();
        //获取月份，获取的月份比实际小1，所以在使用的时候需要+1
        var month = new Date(timdedetail).getMonth() + 1;
        ...
        var time = year + "-" + month + "-" + date + " " + hours + ":" + minutes + ":" + seconds;
        //输出样式：2020-10-25 14:25:36
        arr[i].beginCard=time
    }
}
// 下班打卡
function clockOut(arr){
    for (let i = 0; i < arr.length; i++) {
        var timdedetail = arr[i].endCard;//假如是10位毫秒需要乘1000
        //获取年份
        var year = new Date(timdedetail).getFullYear();
        //获取月份，获取的月份比实际小1，所以在使用的时候需要+1
        var month = new Date(timdedetail).getMonth() + 1;
...
export {dateTime,clockIn,clockOut}