// 计划开始时间
function startTimeStamp(arr){
    for (let i = 0; i < arr.length; i++) {
        var timdedetail = arr[i].planBeginTime;//假如是10位毫秒需要乘1000
        //获取年份
        ...
        arr[i].planBeginTime=time
    }
}
// 计划结束时间
function endTimeStamp(arr){
    for (let i = 0; i < arr.length; i++) {
        var timdedetail = arr[i].planEndTime;//假如是10位毫秒需要乘1000
        //获取年份
        var year = new Date(timdedetail).getFullYear();
        //获取月份，获取的月份比实际小1，所以在使用的时候需要+1
        var month = new Date(timdedetail).getMonth() + 1;
        ...
        var time = year + "-" + month + "-" + date + " " + hours + ":" + minutes + ":" + seconds;
        //输出样式：2020-10-25 14:25:36
        arr[i].planEndTime=time
    }
}
// 更新时间
function updateTimeStamp(arr) {
    for (let i = 0; i < arr.length; i++) {
        var timdedetail = arr[i].updateTime;//假如是10位毫秒需要乘1000
        //获取年份
        var year = new Date(timdedetail).getFullYear();
        //获取月份，获取的月份比实际小1，所以在使用的时候需要+1
        var month = new Date(timdedetail).getMonth() + 1;
...
export {startTimeStamp,endTimeStamp,updateTimeStamp}